...
class MO2Instance(ModInstance):
    ...
    def copy_mods(self, ldialog: LoadingDialog = None):
        ...
        def copy_single_file(task, use_copy_mode):
            nonlocal copied_size, files_processed
            
            # Copy file with appropriate method
            try:
                if use_copy_mode:
                    shutil.copyfile(task['src_path'], task['dst_path'])
                else:  # Link mode
                    os.link(task['src_path'], task['dst_path'])
                
                # Update progress tracking
                with progress_lock:
                    copied_size += task['file_size']
                    files_processed += 1
                    if ldialog:
                ...
                self.log.error(f"Failed to copy {task['src_path']} to {task['dst_path']}: {e}")
                with progress_lock:
                    files_processed += 1
                return False
        
        # Sort tasks to process largest files first for better load balancing
        copy_tasks.sort(key=lambda x: x['file_size'], reverse=True)
        
        # Determine the number of worker threads based on CPU cores
        # Use 75% of available cores to avoid hogging the system
        import multiprocessing
        max_workers = max(1, int(multiprocessing.cpu_count() * 0.75))
        
        # Start time for performance measurement
        start_time = time.time()
        self.log.info(f"Starting parallel copy with {max_workers} workers for {len(copy_tasks)} files...")
        
        # Process files in parallel
        use_copy_mode = (self.app.mode == "copy")
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = [executor.submit(copy_single_file, task, use_copy_mode) for task in copy_tasks]
            concurrent.futures.wait(futures)  # Wait for all tasks to complete
        
        # Write metadata for all mods
        for mod_name, mod_data in mod_metadata.items():
            mod_meta = mod_data["metadata"]
            modpath = mod_data["modpath"]
            ...
            metaini.data = {
                "General": {
                    ...
                    "modid": mod_meta["modid"],
                    "version": mod_meta["version"],
                    "installationFile": f"{mod_meta['filename']}.7z",
                    "repository": "Nexus",
                    "validated": "true",
                },
                "installedFiles": {
                    "1\\modid": mod_meta["modid"],
                    "1\\fileid": mod_meta["fileid"],
                },
            }
            metaini.save_file()
